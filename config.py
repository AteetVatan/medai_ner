...
class ServiceConfig(BaseSettings):
    ...
    spacy_model_name: str = Field(
        default="spacy_de_core_news_md", env="SPACY_MODEL_NAME"
    )
    ...
    gernermed_model_file_name: str = Field(
        default="GERNERMEDpp_GottBERT.zip", env="GERNERMED_MODEL_FILE_NAME"
    )
    gernermed_enabled: bool = Field(default=True, env="GERNERMED_ENABLED")

    germanbert_model_name: Optional[str] = Field(
        default=None, env="GERMANBERT_MODEL_NAME"
    )
    ...
    max_text_length: int = Field(
        default=10000, ge=100, le=100000, env="MAX_TEXT_LENGTH"
    )
    ...
    def validate_germanbert_config(cls, values):
        ...
        if values.get("germanbert_enabled") and not values.get("germanbert_model_url"):
            raise ValueError(
                "GermanBERT URL must be provided when GermanBERT is enabled"
            )
        return values
    ...
    def get_gernermed_model_config(self) -> ModelConfig:
        ...
        return ModelConfig(
            ...
            local_file_path=os.path.join(
                self.models_file_dir, self.gernermed_model_name
            ),
            zip_path=os.path.join(
                self.models_file_dir,
                self.gernermed_model_name,
                self.gernermed_model_file_name,
            ),
    ...
    def get_germanbert_model_config(self) -> Optional[ModelConfig]:
        ...
        return ModelConfig(
            ...
            local_modal_path=os.path.join(
                self.models_dir, self.germanbert_model_name or "GermanBERT"
            ),
...
def validate_config(config: ServiceConfig) -> bool:
    ...
    try:
        ...
        if not (100 <= config.max_text_length <= 100000):
            raise ValueError(
                f"Max text length must be between 100 and 100000, got {config.max_text_length}"
            )
...
ICD_CONFIG = {"lookup_enabled": True, "cache_size": 1000, "timeout": 5}
