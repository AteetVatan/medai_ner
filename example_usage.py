        ...
        async with session.post(
            ...
            if response.status == 200:
                ...
                for entity in result['entities']:
                    icd_info = f" (ICD: {entity['icd_code']})" if entity['icd_code'] else ""
                    category_info = f" [{entity['category']}]" if entity['category'] else ""
                    print(f"  - {entity['text']} ({entity['label']}){category_info}{icd_info} - Confidence: {entity['confidence']:.2f}")
...
async def test_extraction_with_stats():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            "http://localhost:8000/extract_with_stats",
            json={"text": sample_text}
        ) as response:
            if response.status == 200:
                result = await response.json()
...
async def test_batch_extraction():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if response.status == 200:
                result = await response.json()
                print(f"✅ Processed {result['batch_size']} texts in {result['total_processing_time']:.3f}s")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("http://localhost:8000/health") as response:
            if response.status == 200:
                ...
                print(f"⏱️  Average processing time: {health['stats']['avg_processing_time']:.3f}s")
            else:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("http://localhost:8000/stats") as response:
            if response.status == 200:
                ...
                print(f"  - Total extractions: {stats['extraction_stats']['total_extractions']}")
                print(f"  - Total entities: {stats['extraction_stats']['total_entities']}")
                print(f"  - Average processing time: {stats['extraction_stats']['avg_processing_time']:.3f}s")
